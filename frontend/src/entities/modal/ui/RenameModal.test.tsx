...
const renderWithStore = () => {
  ...
  return render(
    <Provider store={store}>
      <RenameModal handleClose={() => {}} toast={() => '1'} />
    </Provider>,
  )
}
...
describe('RenameModal', () => {
  it('renders modal with title and input', () => {
    ...
    expect(screen.getByText('modal.rename')).toBeInTheDocument()
  ...
  it('allows user to enter new channel name', () => {
    renderWithStore()

    const input = screen.getByRole('textbox')
    fireEvent.change(input, { target: { value: 'Renamed Channel' } })

    expect(input).toHaveValue('Renamed Channel')
  })
})
